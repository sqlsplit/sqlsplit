...
$file_list = glob('part-*.sql');
$file = reset($file_list);

foreach (distribute(filesize('a.sql'), 6) as $part_index => $part_size) {
	$part_no = $part_index + 1;
	for ($size = 0; $file && $size < $part_size; $file = next($file_list), $size += filesize($file)) {
		file_put_contents("localhost-$part_no.sql", file_get_contents($file), FILE_APPEND);
	}
}
